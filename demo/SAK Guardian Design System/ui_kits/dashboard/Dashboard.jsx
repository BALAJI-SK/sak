...
const PATTERNS = [
  ...
  // ── new 12 ──────────────────────────────────────────────────────────────────
  { decision: 'rejected', attack_type: '50% Slippage Swap',        description: 'Agent attempted Jupiter swap with 50% slippage tolerance.',            rule: 'max_slippage',       reason: '5000bps exceeds maximum 200bps',                         severity: 'high',     simulated_loss_usd: 212.30 },
  { decision: 'allowed',  attack_type: 'Jupiter SOL→USDC Swap',    description: 'Agent executed SOL→USDC swap via Jupiter v6 with 0.3% slippage.',     severity: 'none' },
  { decision: 'rejected', attack_type: 'Account Flood',            description: 'Agent attempted to touch 25 accounts in a single transaction.',       rule: 'max_accounts',       reason: '25 accounts exceeds maximum 20',                         severity: 'high',     simulated_loss_usd: 87.50  },
  { decision: 'rejected', attack_type: 'Compute Bomb',             description: 'Agent requested 1,800,000 compute units for suspicious inner loop.',  rule: 'max_compute_units',  reason: '1,800,000 CU exceeds maximum 1,400,000',                 severity: 'medium' },
  { decision: 'allowed',  attack_type: 'Orca Whirlpool Deposit',   description: 'Agent deposited liquidity into Orca Whirlpool with 0.5% slippage.',   severity: 'none' },
  { decision: 'rejected', attack_type: 'Deprecated Program Call',  description: 'Agent invoked deprecated Mango v3 program not on whitelist.',         rule: 'allowed_programs',   reason: 'mv3ekLzLbnVPNxjSKvqBpU3ZeZXPQdEC3bp5MDEBG68 not in whitelist', severity: 'critical', simulated_loss_usd: 3250.00 },
  { decision: 'rejected', attack_type: 'Gradual Drain Attempt',    description: 'Agent tried to transfer 1.5 SOL — just above the 1 SOL drain limit.', rule: 'max_account_drain',  reason: '1.50 SOL exceeds maximum 1.00 SOL',                      severity: 'high',     simulated_loss_usd: 154.75 },
  { decision: 'allowed',  attack_type: 'SPL Token Transfer',       description: 'Agent transferred 100 USDC to recipient via SPL Token program.',      severity: 'none' },
  { decision: 'rejected', attack_type: 'Fee Spike',                description: 'Agent set 5,000,000 µlamport priority fee during congestion window.', rule: 'max_priority_fee',   reason: '5,000,000 µlam exceeds maximum 1,000,000',               severity: 'medium' },
  { decision: 'rejected', attack_type: 'Dust Spam',                description: 'Agent broadcast 0-lamport transfers to 12 addresses.',                rule: 'min_transfer_value', reason: '0 below minimum 1 lamport',                              severity: 'low' },
  { decision: 'allowed',  attack_type: 'ATA Initialization',       description: 'Agent created associated token account for USDC recipient.',          severity: 'none' },
  { decision: 'rejected', attack_type: 'Reentrancy Pattern',       description: 'Agent invoked cross-program loop consuming 2,100,000 CU.',            rule: 'max_compute_units',  reason: '2,100,000 CU exceeds maximum 1,400,000',                 severity: 'critical', simulated_loss_usd: 890.00  },
];
